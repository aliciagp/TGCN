...
#' filterNonCodingRNAs - It filters the expression matrix to keep only non coding RNAs.
#'
#' @param exprData a data frame representing a gene expression matrix with genes as rows and samples as columns
#' @returna a filtered data frame containing only non coding RNAs transcripts.
#' @import biomaRt
#' @export

filterNonCodingRNAs <- function(exprData) {
  # Load biomaRt
  require(biomaRt)
  
  mart <- biomaRt::useMart(biomart="ENSEMBL_MART_ENSEMBL", dataset="hsapiens_gene_ensembl")
  
  # Get gene annotations for all genes in the expression matrix
  genes <- biomaRt::getBM(attributes=c("mirbase_id", "mirbase_trans_name", "ensembl_gene_id",
                                       "external_gene_name", "gene_biotype"),
                          filters="external_gene_name",
                          values=rownames(exprData),
                          mart=mart,
                          useCache=FALSE)
  
  # simple classification: protein_coding vs non-coding
  genes$gene_type <- ifelse(genes$gene_biotype == "protein_coding", "protein_coding", "non-coding")
  
  # duplicates
  genes_unique <- genes[!duplicated(genes$external_gene_name), c("external_gene_name", "gene_type")]
  
  # Add gene names
  exprData_with_info <- exprData
  exprData_with_info$gene_name <- rownames(exprData)
  
  # Merge the expression data with gene type information
  exprData_with_info <- merge(exprData_with_info,
                              genes_unique,
                              by.x = "gene_name",
                              by.y = "external_gene_name",
                              all.x = TRUE)
  
  # Set row names back to gene names
  rownames(exprData_with_info) <- exprData_with_info$gene_name
  
  # Filter to keep only non-coding RNAs
  exprData_filtered <- exprData_with_info[exprData_with_info$gene_type == "non-coding", ]
  
  # Remove the gene_type and gene_name columns
  exprData_filtered <- exprData_filtered[, !colnames(exprData_filtered) %in% c("gene_type", "gene_name")]
  exprData_filtered <- na.omit(exprData_filtered)
  return(exprData_filtered)
}
...
getLASSOmodels <- function(exprData,
  ...
  for (iter in 1:t) {
    ...
    # Included "else" to deal with non factor targets:
    if(is.factor(target)) {
...
getHubGenes <- function(exprData,
  ...
  if(is.na(file)) {
    ...
    # Check the condition for the number of transcripts
    if(length(count) <= 1) {
      stop("The number of transcripts selected for all cutoffs is less than 2.")
    }
    if(length(count)==0) {
...
#' @param tissueName the name of the tissue, cohort or dataset
#' @param targetName the name of the target
#' @param useNonCodingOnly if useNonCodingOnly == T, modules annotation via enrichment would be ignored, as we are working with RNAs.
...
getModulesAnnotation <- function(net,
  ...
  if(is.na(files) || overwrite==T) {
    ...
    if(!is.null(enrich$sumLogPval)) {
      ...
      terms <- enrich$enrichment[, -ncol(enrich$enrichment)]
    ...
    # Get CT enrichment just if we are using coding genes. 
    if (useNonCodingOnly == F) {
    ...
    if(save==T) {
      if (!is.null(terms)) {
        list_cols <- sapply(terms, is.list)
        terms[list_cols] <- lapply(terms[list_cols], function(col) {
    ...
    result <- list(net=list(modules=mynet, MEs=MEs,
                   ...
                   GOenrich=list(terms=terms,
                                 stats=enrich$sumLogPval,
                                 reducedTerms=reducedTerms,
                                 plotNterms=plotNterms,
                                 plotStats=plotStats,
                                 plotReduced=plotReduced),
...
#' @param save if save=T, the results of the analysis will be saved in separate files
#' @param overwrite if overwrite=T, the analysis will be repeated and files will be overwritten
#' @param useNonCodingOnly if useNonCodingOnly == T, the exprData will be filtered to keep only non coding genes and modules annotation will be ignored.
#' @param path the path where results are stored
#' @return
#' @export
#' @examples
...
testAllCutoffs <- function(exprData,
  ...
  # Filter exprData to include only non-coding RNA transcripts if requested
  if (useNonCodingOnly==TRUE) {
    exprData <- filterNonCodingRNAs(exprData)
    cat("Filtered expression data to include only non-coding RNA transcripts. \n")
    cat(paste("Number of non-coding transcripts used:", nrow(filteredexprData)))
  }
  ...
  if(report==T) {
    ...
    # template="D:/DESCARGAS/TFM/mTGCN/template.Rmd"
    # template is missing!
